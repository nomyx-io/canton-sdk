...
class CantonClient {
        ...
        for (let attempt = 0; attempt <= this.retryConfig.maxRetries; attempt++) {
            ...
            try {
                ...
                if (err instanceof DOMException && err.name === 'TimeoutError') {
                    lastError = new errors_js_1.ConnectionError(`Ledger API request timed out after ${this.requestTimeoutMs}ms: ${url}`, err);
                }
                else {
                    lastError = new errors_js_1.ConnectionError(`Failed to reach ledger API at ${url}`, err);
                }
                continue;
            }
            let json;
            ...
            if (!isJson) {
                lastError = new errors_js_1.HttpError(`Ledger API returned non-JSON response (${res.status})`, res.status, url);
                if (res.status >= 500)
                    continue;
                throw lastError;
            }
            ...
            lastError = new errors_js_1.LedgerApiError(`Ledger API ${path}: ${errors.join('; ')}`, path, json.status ?? res.status, errors);
    ...
    assertNonEmpty(value, name) {
        if (!value || value.trim() === '') {
            throw new errors_js_1.CantonError(`${name} must be a non-empty string`);
        }
    }
    // ---------------------------------------------------------------------------
    // Contract operations
    ...
    async exerciseByKey(templateId, key, choice, argument = {}, actAs, readAs) {
        ...
        if (key === null || key === undefined) {
            throw new errors_js_1.CantonError('key must not be null or undefined');
        }
        const token = await this.actorToken(actAs, readAs);
        return this.callApi('/v1/exercise', token, {
            templateId,
